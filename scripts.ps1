...
az aks create -n $clusterName -g $group --network-plugin azure --enable-oidc-issuer --enable-workload-identity -c 2
...
helm install alb-controller oci://mcr.microsoft.com/application-lb/charts/alb-controller --namespace "alb" --version 1.0.2 --set albController.namespace="alb" --set albController.podIdentity.clientID=$(az identity show -g $group -n $identityName --query clientId -o tsv)
